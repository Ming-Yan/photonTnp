...
//float PhotonIDCutBasedRunII(int iWP, TreeReader &data, int i); //remove later
//float PhotonIDMVA(TreeReader &data, int i); //remove later
...
void xAna(const char** input, int npaths, const char* output = "preminitree.root",
	...
	float L = (3976412368.912) / (1000. * 1000. * 1000.);
	float mcwei = 1.;

	TString stype = type;

	if (stype == "signal") mcwei = (6025.2 * 1000.)*L / 19223750.;
	if (stype == "WgBkg") mcwei = (60290 * 1000.)*L / 6776900.;
	if (stype == "TTBkg") mcwei = (730 * 1000.)*L / 93002283.;
	if (stype == "signal50120") mcwei = (1975 * 1000.)*L / 2967200.;
	if (stype == "signal120200") mcwei = (19.32 * 1000.)*L / 99200.;
	...
	TTree *outtree_ = new TTree("t", "mini tree"); //Shilpi used "EventTree", "Intermediate TTree"

	Int_t run;

	//TREE VARIABLES
	int EleVeto_, HasPix_;
	int looseID_, medID_, tightID_, bMVA_, presel_;//1 means pass
	float MVA_, SCEta_;
	...
	if (data.HasMC()) {//initialize using reference files
		puCalc.Init("PU_histo_13TeV_GoldenJSON_69000nb.root");
		puCalc_71.Init("PU_histo_13TeV_GoldenJSON_71300nb.root");
		puCalc_up.Init("PU_histo_13TeV_GoldenJSON_74865nb.root");
		puCalc_down.Init("PU_histo_13TeV_GoldenJSON_67735nb.root");
	}
	...
	for (Long64_t ev = ev1; ev < ev2; ev++) { //ev1 = 0, ev2 = -1 by deafult; ev1 -> lower limit; ev2 -> Nevents
		...
		genWeight_ = 1.; //deafult genWeight; for data
