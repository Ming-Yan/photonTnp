...
void xHistosCombine(TString fname="Loose", TString region="EB", TString hname="Muu", TString xname = "x", Int_t legStyle=0, Int_t logy=0, Int_t logx=0) {
	...
	TFile *f2 = new TFile("../WgBkgplots" + fname + region + ".root");
	TFile *f3 = new TFile("../TTBkgplots" + fname + region + ".root");
	...
	double L = (3976412368.912) / (1000. * 1000. * 1000.);
	hs->Scale((6025.2 * 1000.)*L / 19223750.);
	hwg->Scale((60290 * 1000.)*L / 6776900.);
	htt->Scale((730 * 1000.)*L / 93002283.);

	hs->SetFillColor(5);
	htt->SetFillColor(kViolet);
	hwg->SetFillColor(kTeal);

	THStack *s = new THStack("s", "MC");
	s->Add(hs, "hist");
	s->Add(htt, "hist");
	s->Add(hwg, "hist");

	hs->Add(htt); //for Data/MC
	hs->Add(hwg);
