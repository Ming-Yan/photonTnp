...
void analyse3plots(TString &IDtype, TString &CutType, TString &R9Type, Int_t &puType){
	...
	//PLOTS
	TH1F *hMuu = new TH1F("hMuu", "M_{#mu#mu}", 100, 20, 90);
	TH1F *hMuug = new TH1F("hMuug", "M_{#mu#mu#gamma}", 100, 70, 100);
	TH1F *Ptg = new TH1F("Ptg", "P_{T}^{#gamma}", 100, 0, 100);
	TH1F *Etag = new TH1F("Etag", "#eta^{#gamma}", 100, -3, 3);
	TH1F *phoMVA = new TH1F("phoMVA", "Photon ID MVA", 100, -1, 1);
	TH2F* MuugMuu = new TH2F("MuugMuu", "M_{#mu#mu#gamma} Vs. M_{#mu#mu}", 100, 70, 140, 100, 70, 100);

	TH1F *shMuu = new TH1F("shMuu", "M_{#mu#mu}", 100, 20, 90);
	TH1F *shMuug = new TH1F("shMuug", "M_{#mu#mu#gamma}", 100, 70, 100);
	TH1F *sPtg = new TH1F("sPtg", "P_{T}^{#gamma}", 100, 0, 100);
	TH1F *sEtag = new TH1F("sEtag", "#eta^{#gamma}", 100, -3, 3);
	TH1F *sphoMVA = new TH1F("sphoMVA", "Photon ID MVA", 100, -1, 1);
	TH2F *sMuugMuu = new TH2F("sMuugMuu", "M_{#mu#mu#gamma} Vs. M_{#mu#mu}", 100, 70, 140, 100, 70, 100);

	double etabin[4] = { 0.0, 1.4442, 1.566, 2.5 }; //0.0, 1.4442, 1.566, 2.5
	...
	//NUMERATORS
	TH1F* hs = new TH1F("hs", "signal_" + IDtype + "_" + CutType + "_" + R9Type, 3, etabin);
	TH1F* hd = new TH1F("hd", "data_" + IDtype + "_" + CutType + "_" + R9Type, 3, etabin);

	//DENOMINATORS
	TH1F* dhs = new TH1F("dhs", "den_signal_" + IDtype + "_" + CutType + "_" + R9Type, 3, etabin);
	TH1F* dhd = new TH1F("dhd", "den_data_" + IDtype + "_" + CutType + "_" + R9Type, 3, etabin);

	//SF (eff data/eff signal)
	TH1F* hsf = new TH1F("hsf", "SF_" + IDtype + "_" + CutType + "_" + R9Type, 3, etabin);

	//2D
	//NUMERATORS
	TH2F* hs2 = new TH2F("hs2", "signal_" + IDtype + "_" + CutType + "_" + R9Type, 3, etabin, 1, ptbin);
	TH2F* hd2 = new TH2F("hd2", "data_" + IDtype + "_" + CutType + "_" + R9Type, 3, etabin, 1, ptbin);

	//DENOMINATORS
	TH2F* dhs2 = new TH2F("dhs2", "den_signal_" + IDtype + "_" + CutType + "_" + R9Type, 3, etabin, 1, ptbin);
	TH2F* dhd2 = new TH2F("dhd2", "den_data_" + IDtype + "_" + CutType + "_" + R9Type, 3, etabin, 1, ptbin);

	//SF (eff data/eff signal)
	TH2F* hsf2 = new TH2F("hsf2", "SF_" + IDtype + "_" + CutType + "_" + R9Type, 3, etabin, 1, ptbin);
	...
	for (Long64_t i = 0; i < nentries; i++) {
		...
		if (fabs(SCEta[i]) > 1.4442 && fabs(SCEta[i]) < 1.566) continue;
		if (!bID[i]) continue;
	...
	for (Long64_t si = 0; si < snentries; si++) {
		...
		if (fabs(SCEta[si]) > 1.4442 && fabs(SCEta[si]) < 1.566) continue;
		if (!bIDs[si]) continue;
	...
	for (Int_t nbin = 0; nbin < 3; nbin++){
		if (nbin == 1){
			//hd2->SetBinError(2, 1, 0);
			//hs2->SetBinError(2, 1, 0);
			continue;
		}
		...
		if (nbin == 2){
			double sefferr = TMath::Max(seff->GetErrorYhigh(1), seff->GetErrorYlow(1));
			double defferr = TMath::Max(deff->GetErrorYhigh(1), deff->GetErrorYlow(1));
		}

		hd2->SetBinError(nbin + 1, 1, defferr);
		hs2->SetBinError(nbin + 1, 1, sefferr);

		double den = hs->GetBinContent(nbin + 1);//signal denominator
		double num = hd->GetBinContent(nbin + 1);

		sferr[nbin] = (num / den)*sqrt(pow(defferr / num, 2) + pow(sefferr / den, 2));
	}
	...
	for (Int_t nbin = 0; nbin < 3; nbin++){
		...
		hsf2->SetBinError(nbin + 1, 1, sferr[nbin]);
		//cout << hs2->GetBinContent(nbin + 1, 1) << " " << hs2->GetBinError(nbin + 1, 1) << endl;
	}
	...
	hs2->Draw("colztexte");
	c->Print("plots/signal_" + IDtype + "_" + CutType + "_" + R9Type + ".png");

	TCanvas *d = new TCanvas("d", "d", 900, 600);
	d->cd();
	hd2->SetMarkerSize(2.5);
	hd2->Draw("colztexte");
	d->Print("plots/data_" + IDtype + "_" + CutType + "_" + R9Type + ".png");

	TCanvas *e = new TCanvas("e", "e", 900, 600);
	e->cd();
	hsf2->SetMarkerSize(2.5);
	hsf2->Draw("colztexte");
	e->Print("plots/SF_" + IDtype + "_" + CutType + "_" + R9Type + ".png");
