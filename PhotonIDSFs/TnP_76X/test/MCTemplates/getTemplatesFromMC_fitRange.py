...
def main(options):
    ...
    for binPt in xrange(len(pts)-1):
        for binEta in xrange(len(etas)-1):
            ...
            hp = histNameSt+"_Pass"
            hf = histNameSt+"_Fail"
            histos[hp] = ROOT.TH1D(hp, hp, 120, 60, 140)
            histos[hf] = ROOT.TH1D(hf, hf, 120, 60, 140)
            ...
            ###SJ : 28th feb, 2016
            #binning = "pair_mass60to120 && tag_Pho_et > 30 && " +options.etVarName +">"+str(pts[binPt])+" && "+options.etVarName +"<"+str(pts[binPt+1])+" && "+options.etaVarName +">"+str(etas[binEta])+" && "+options.etaVarName +"<"+str(etas[binEta+1])            
            binning = "pair_mass>=60 && pair_mass<=140 && tag_Pho_et > 30 && " +options.etVarName +">"+str(pts[binPt])+" && "+options.etVarName +"<"+str(pts[binPt+1])+" && "+options.etaVarName +">"+str(etas[binEta])+" && "+options.etaVarName +"<"+str(etas[binEta+1])            
...
if __name__ == "__main__":  
    parser = OptionParser()
    parser.add_option("-i", "--input", default="/data2/pwang/TnP/76X_v2/TnP_MC_PO.root", help="Input filename")
    ...
    parser.add_option("", "--idprobe", default="Tight", help="String identifying ID WP to measure")
