...
process.source = cms.Source("PoolSource",
    ...
    fileNames = cms.untracked.vstring('/store/mc/RunIISpring16MiniAODv1/DYToEE_NNPDF30_13TeV-powheg-pythia8/MINIAODSIM/PUSpring16_80X_mcRun2_asymptotic_2016_v3-v1/00000/00271303-C80D-E611-A98C-0002C94D552A.root', 
...
process.goodPhotonsPROBECutBasedLoose = cms.EDProducer("PatPhotonSelectorByValueMap",
    cut = cms.string('(abs(-log(tan(superCluster.position.theta/2)))<=2.5) && pt> 10 && ((abs(eta)<1.4442 && full5x5_sigmaIetaIeta < 0.015) || (abs(eta)>1.56 && (abs(eta)<2.5 && full5x5_sigmaIetaIeta < 0.045)) )'),
    id_cut = cms.bool(True),
...
process.pileupReweightingProducer = cms.EDProducer("PileupWeightProducer",
    PileupData = cms.vdouble(2340.0, 77000.0, 371000.0, 777000.0, 1170000.0, 
        1640000.0, 2750000.0, 13400000.0, 39100000.0, 80000000.0, 
        138000000.0, 194000000.0, 257000000.0, 341000000.0, 432000000.0, 
        508000000.0, 553000000.0, 563000000.0, 542000000.0, 498000000.0, 
        435000000.0, 360000000.0, 279000000.0, 201000000.0, 133000000.0, 
        82300000.0, 47500000.0, 25800000.0, 13300000.0, 6590000.0, 
        3160000.0, 1470000.0, 665000.0, 291000.0, 124000.0, 
        53000.0, 24000.0, 12800.0, 8590.0, 7040.0, 
        6420.0, 6130.0, 5950.0, 5790.0, 5640.0, 
        5470.0, 5270.0, 5040.0, 4790.0, 4510.0),
...
process.ele_sequence = cms.Sequence(process.goodElectronsPROBECutBasedVeto+process.goodElectronsPROBECutBasedLoose+process.goodElectronsPROBECutBasedMedium+process.goodElectronsPROBECutBasedTight+process.goodElectronsProbeHLT)
...
process.pho_sequence = cms.Sequence(process.goodPhotons+process.egmPhotonIDSequence+process.photonIDValueMapProducer+process.goodPhotonsPROBECutBasedLoose+process.goodPhotonsPROBECutBasedMedium+process.goodPhotonsPROBECutBasedTight+process.goodPhotonsPROBEMVA+process.goodPhotonsProbeHLT+process.goodPhotonsPhoIso+process.goodPhotonsPROBEFullPreselection)


process.tag_sequence = cms.Sequence(process.goodElectrons+process.egmGsfElectronIDSequence+process.goodElectronsTAGCutBasedLoose+process.goodElectronsTAGCutBasedTight+process.goodElectronsTagHLT)


process.cand_sequence = cms.Sequence(process.tag_sequence+process.pho_sequence)
