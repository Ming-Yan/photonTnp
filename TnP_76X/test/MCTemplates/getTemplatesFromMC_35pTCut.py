...
def main(options):
    ...
    for binPt in xrange(len(pts)-1):
        for binEta in xrange(len(etas)-1):
            ...
            ###SJ : 28th feb, 2016
            binning = "pair_mass60to120 && tag_Pho_et > 35 && " +options.etVarName +">"+str(pts[binPt])+" && "+options.etVarName +"<"+str(pts[binPt+1])+" && "+options.etaVarName +">"+str(etas[binEta])+" && "+options.etaVarName +"<"+str(etas[binEta+1])            
